...
fn main() {
  ...
  let tap = TapInterface::new(&tap_text).expect("error: unable to use <tap-device> as a \
                                                 network interface");

  let domain_name = url.host_str().expect("domain name required");

  let _dns_server: std::net::Ipv4Addr = dns_server_text.parse().expect("error: unable to parse <dns-server> as an \
                                                                        IPv4 address");

  let addr = dns::resolve(dns_server_text, domain_name).unwrap();

  let mac = ethernet::MacAddress::new().into();

  println!("{:?}", http::get(tap, mac, addr.unwrap(), url).unwrap());
}
